...
public function GetMargins() {
    // Devuelve los márgenes actuales como un array
    return [
        'left' => $this->lMargin,
        'top' => $this->tMargin,
        'right' => $this->rMargin,
...
function Output($dest='', $name='', $isUTF8=false)
{
	...
	switch(strtoupper($dest))
	{
		case 'I':
			...
			// Try to log inline PDF view
			if (session_status() === PHP_SESSION_NONE) { @session_start(); }
			@include_once __DIR__ . '/../../../inicio_sesion/seguridad/log_registros.php';
			if (function_exists('app_log')) {
				app_log('export', 'pdf', "Visualización PDF: $name", ['mode' => 'inline', 'actor' => $_SESSION['username'] ?? '']);
			}
			echo $this->buffer;
			break;
		case 'D':
			...
			header('Cache-Control: private, max-age=0, must-revalidate');
			header('Pragma: public');
				// Try to log PDF download
				if (session_status() === PHP_SESSION_NONE) { @session_start(); }
				@include_once __DIR__ . '/../../../inicio_sesion/seguridad/log_registros.php';
				if (function_exists('app_log')) {
					app_log('export', 'pdf', "Descarga PDF: $name", ['mode' => 'download', 'actor' => $_SESSION['username'] ?? '']);
				}
				echo $this->buffer;
			break;
		case 'F':
